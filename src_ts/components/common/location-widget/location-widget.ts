...
export class LocationWidgetComponent extends LitElement {
    @property() public selectedLocation: string | null = null;
    @property() public selectedSites: number[] = [];
    @property({ type: Boolean, attribute: 'multiple-sites' }) public multipleSites: boolean = false;
    ...
    public connectedCallback(): void {
        ...
        this.widgetLoadingUnsubscribe = store.subscribe(widgetLocationsLoading((loading: boolean | null) => {
            if (typeof loading !== 'boolean') { return; }
            this.listLoading = loading;
        }, false));

        this.pathLoadingUnsubscribe = store.subscribe(widgetLocationPathLoading((pathLoading: boolean | null) => {
            if (typeof pathLoading !== 'boolean') { return; }
    ...
    public getListItems(path: string, loading?: boolean): (Site | WidgetLocation)[] {
        return !loading && path && this.widgetLocations[path] || [];
    }

    public onLocationLineClick(location: WidgetLocation): void {
        this.history = [...this.history, location];
        this.selectLocation(location);
        this.locationSearch = '';
    }

    public onSiteLineClick(location: Site): void {
    ...
    public onSiteHoverStart(location: Site): void {
    ...
    public onSiteHoverEnd(): void {
        this.MapHelper.removeDynamicMarker();
    }

    public getSiteLineClass(siteId: number | string): string {
    ...
    public restoreHistory(locationId: string | null, loading?: boolean): void {
    ...
    public locationsFilter(location: Site | WidgetLocation): boolean {
        return !this.locationSearch || location.name
            .toLocaleLowerCase()
            .indexOf(this.locationSearch.toLocaleLowerCase()) !== -1;
    }

    public getHistoryInputLabel(gateway: LocationGateway): string {
    ...
    public getLocationPart(location: string = '', partToSelect: string): string {
        return getLocationPart(location, partToSelect);
    }

    public hideEmptySitesMessage(path: string, loading: boolean): boolean {
        const lastLocation: WidgetLocation | null = this.getLastLocation();
    ...
    protected updated(changedProperties: PropertyValues): void {
        ...
        const locationOrLoadingChanged: boolean = properties.some((propertyName: string) => changedProperties.has(propertyName));
    ...
    private checkSelectedSites(selectedSites: number[], selectedLocation: string | null): void {
        ...
        if (this.sitesLoading || !selectedSites.length) { return; }
        ...
        if (missingSites.length !== 0) {
            ...
            this.selectedSites = selectedSites.filter((siteId: number) => missingSites.indexOf(siteId) === -1);
    ...
    private centerAndDrawBorders(location: WidgetLocation): void {
        ...
        const reversedCoordinates: CoordinatesArray[] = coordinates.map((coordinate: CoordinatesArray) => [...coordinate].reverse() as CoordinatesArray);
    ...
    public static get styles(): CSSResultArray {
        return [elevationStyles, SharedStyles, pageLayoutStyles, FlexLayoutClasses, CardStyles,
            TemplatesStyles, LocationWidgetStyles, leafletStyles];
    }
