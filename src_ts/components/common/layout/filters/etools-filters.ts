...
export class EtoolsFilters extends LitElement {
  ...
  set filters(filters: IEtoolsFilter[]) {
  ...
  get selectedFiltersTmpl(): TemplateResult {
    return html`
      ${repeat(
  ...
  get filterMenuOptions(): TemplateResult[] {
  ...
  getSearchTmpl(f: IEtoolsFilter): TemplateResult {
    // language=HTML
    return html`
      <paper-input
        class="filter search"
  ...
  getDropdownTmpl(f: IEtoolsFilter): TemplateResult {
    // language=HTML
    return html`
      <etools-dropdown
        class="filter"
  ...
  getDropdownMultiTmpl(f: IEtoolsFilter): TemplateResult {
    // language=HTML
    return html`
  ...
  getDateTmpl(f: IEtoolsFilter): TemplateResult {
    // language=HTML
    return html`
      <datepicker-lite
        class="filter date"
  ...
  getToggleTmpl(f: IEtoolsFilter): TemplateResult {
    // language=HTML
    return html`
  ...
  render(): TemplateResult {
    ...
    return html`
      ...
      <div id="filters">${this.selectedFiltersTmpl}</div>
  ...
  selectFilter(e: CustomEvent): void {
  ...
  getFilterEmptyValue(filterType: EtoolsFilterTypes): '' | boolean | null | [] {
    switch (filterType) {
      case EtoolsFilterTypes.Search:
        return '';
      case EtoolsFilterTypes.Toggle:
  ...
  getFilterOption(filterElement: HTMLElement): IEtoolsFilter {
    ...
    const filterOption: IEtoolsFilter | undefined = this.filters.find((f: IEtoolsFilter) => f.filterKey === filterKey);
  ...
  filterMultiSelectionChange(e: CustomEvent): void {
    const filterEl: HTMLElement = e.currentTarget as HTMLElement;
    const filterOption: IEtoolsFilter = this.getFilterOption(filterEl);
    const currentSelectedVal: boolean =
