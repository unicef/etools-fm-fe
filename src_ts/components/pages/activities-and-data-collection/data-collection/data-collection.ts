...
export class DataCollectionFinding extends MethodsMixin(LitElement) {
            ...
            <page-content-header>
                <div slot="page-title">
                    ...
                    <div class="title-description">
                        <a href="${ROOT_PATH}${PAGE}/${this.activityId}/${DETAILS_TAB}">${ this.activityDetails && this.activityDetails.reference_number || '' }</a> |
                        ${ this.checklist && this.checklist.id } |
                        ${ this.checklist && this.checklist.author.name } </div>
                </div>
            </page-content-header>
        `;
    }
    ...
    public connectedCallback(): void {
        ...
        this.checklistUnsubscribe = store.subscribe(dataCollectionChecklistData((data: DataCollectionChecklist | null) => {
            if (!data) { return; }
            this.checklist = data;
        }, false));
        ...
        this.routeDetailsUnsubscribe = store.subscribe(routeDetailsSelector(({ routeName, subRouteName, params }: IRouteDetails) => {
            ...
            if (!activityId || !checklistId) { updateAppLocation('page-not-found'); }
    ...
    private loadActivityDetails(): void {
        if (this.activityId === null) { return; }
    ...
    private loadChecklist(): void {
        if (this.activityId === null || this.checklistId === null || this.activityDetails === null) { return; }
        ...
        const loadedChecklistId: number | null = dataCollectionState.checklist.data &&
            dataCollectionState.checklist.data.id;
        const isNotLoaded: boolean = !loadedChecklistId || `${ loadedChecklistId }` !== `${ this.checklistId }`;

        if (isNotLoaded) {
            store.dispatch<AsyncEffect>(loadDataCollectionChecklistInfo(this.activityId as string, this.checklistId));
        } else {
    ...
    public static get styles(): CSSResultArray {
        // language=CSS
        return [SharedStyles, pageContentHeaderSlottedStyles, pageLayoutStyles, buttonsStyles, css`
            page-content-header {
