            ...
            <div slot="content" class="table-container">
                ...
                <div class="details-input elevation" elevation="2" ?hidden="${ !this.editedDetails.opened }" style="${ this.getDetailsInputStyles() }">
                    <paper-textarea id="details-input"
                                 ...
                                 @blur="${ () => this.updateItemDetails(this.editedDetails.id, this.editedDetails.details) }"></paper-textarea>
                </div>
                ...
                <etools-data-table-header no-title no-collapse>
                    <div class="checkbox-container">
                        <paper-checkbox ?checked="${ this.allQuestionsEnabled }" class="nolabel"
                                        ?hidden="${ !this.isEditMode }"
                                        @change="${ (event: CustomEvent) => this.toggleAll((event.target as PaperCheckboxElement).checked as boolean) }"></paper-checkbox>
                    </div>
                ...
                <!-- Table Row item -->
                ${ this.questionsList.map((question: IChecklistItem) => html`
                    <etools-data-table-row no-collapse>
                        <div slot="row-data" class="layout horizontal editable-row flex">

                            <!-- Checkbox to mark question  as enabled -->
                            <div class="checkbox-container layout horizontal center-center">
                                <paper-checkbox ?checked="${ question.is_enabled }" class="nolabel"
                                        ?hidden="${ !this.isEditMode }"
                                        @change="${ (event: CustomEvent) => { question.is_enabled = (event.target as PaperCheckboxElement).checked as boolean; this.requestUpdate(); } }"></paper-checkbox>
                            ...
                            <div class="col-data flex-2 truncate ${ this.isEditMode ? 'edited-col' : '' }"
                                @click="${ ({ currentTarget }: CustomEvent) => this.showDetailsInput(currentTarget as HTMLElement, question.id, question.specific_details) }">
                                ${ question.specific_details ||
                                                ( this.isEditMode ? html`<span class="detail-placeholder">${ translate('ACTIVITY_CHECKLIST.ADD_DETAIL') }</span>` : '-' ) }
                            </div>

                            <!-- Question Methods -->
                            <div class="col-data flex-none w210px truncate methods">${ this.serializeMethods(question.question.methods) }</div>
                        </div>
