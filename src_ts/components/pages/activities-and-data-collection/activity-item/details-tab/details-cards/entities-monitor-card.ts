...
export class EntitiesMonitorCard extends PartnersMixin(CpOutputsMixin(BaseDetailsCard)) {
    @property() public activityPartners: EtoolsPartner[] = [];
    @property() public activityCpOutputs: EtoolsCpOutput[] = [];
    @property() public activityInterventions: EtoolsIntervention[] = [];
    ...
    public set data(data: IActivityDetails) {
        super.data = data;
    ...
    public openAddPartner(): void {
        openDialog({ dialog: 'partner-popup' })
            .then(({ confirmed, response }: IDialogResponse<EtoolsPartner>) => {
                if (!confirmed) { return; }
                if (response) {
    ...
    public openAddCpOutput(): void {
        console.log('add cp output');
    }

    public openAddIntervention(): void {
        console.log('add intervention');
    }

    public removeItem<T extends EtoolsCpOutput | EtoolsPartner | EtoolsIntervention>(
        id: number, field: keyof IActivityDetails,
    ...
    public static get styles(): CSSResultArray {
        // language=CSS
        return [elevationStyles, SharedStyles, FlexLayoutClasses, css`
            .card-content {
                padding: 25px 18px;
            }
            .entries-list {
