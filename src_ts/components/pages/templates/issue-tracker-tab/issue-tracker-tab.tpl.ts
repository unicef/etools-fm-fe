...
export function template(this: IssueTrackerTabComponent): TemplateResult {
  ...
  return html`
    <section class="elevation page-content filters layout horizontal" elevation="1">
        <div class="filter">
            <etools-dropdown-multi
                ...
                @etools-selected-items-changed="${({detail}: CustomEvent) =>
                  this.onOutputsChanged(detail.selectedItems)}"
                .minWidth="160px"
                .autoWidth="${true}"
        ...
        <div class="filter">
            <etools-dropdown-multi
                ...
                trigger-value-change-event
                @etools-selected-items-changed="${({detail}: CustomEvent) =>
                  this.onPartnersChanged(detail.selectedItems)}"
                min-width="160px"
                .autoWidth="${true}"
                horizontal-align="left"
        ...
        <div>
            <etools-dropdown-multi
                ...
                trigger-value-change-event
                @etools-selected-items-changed="${({detail}: CustomEvent) =>
                  this.onLocationsChanged(detail.selectedItems)}"
                .minWidth="160px"
                .autoWidth="${true}"
                horizontal-align="left"
                no-dynamic-align></etools-dropdown-multi></div>
        <div class="toggle-button-control filter">
            <paper-toggle-button
                ...
                @checked-changed="${({detail}: CustomEvent) =>
                  this.changeShowOnlyNew(detail.value)}"></paper-toggle-button>
    ...
    <section class="elevation page-content card-container issue-tracker-table-section" elevation="1">
        <etools-loading ?active="${this.isLoad}" loading-text="${translate(
    'MAIN.LOADING_DATA_IN_PROCESS'
  )}"></etools-loading>
