...
function validateTranslateObject(translateObject: TranslateObject): boolean {
  ...
  return Object.values(translateObject).every(
    (value: any) => typeof value === 'string' || validateTranslateObject(value)
  );
}
...
export function translate(path: string, data?: GenericObject): string {
  ...
  return !data
    ? result
    : Object.entries(data).reduce((resultString: string, [key, value]: [string, any]) => {
