...
function composedDynamicSelector<S, T>(selector: StoreSelectorFunction<S>, path: string[]): StoreSelectorFunction<T | undefined> {
    return (rootState: IRootState) => path.reduce((state: any, pathItem: string) => state && state[pathItem] || undefined, selector(rootState));
}
