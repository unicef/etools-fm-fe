...
export function loadQuestionTemplates(params: IRouteQueryParams, level: string, target?: string): (dispatch: Dispatch) => Promise<void> {
    return (dispatch: Dispatch) => {
        ...
        return request<IListData<IQuestionTemplate>>(resultUrl, { method: 'GET' })
            .then((response: IListData<IQuestionTemplate>) => { dispatch(new SetQuestionTemplatesList(response)); });
    };
}
