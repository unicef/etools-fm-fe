...
export function loadActivitiesList(_params: IRouteQueryParams): (dispatch: Dispatch) => Promise<void> {
    return (dispatch: Dispatch) => {
        ...
        return request<IListData<IListActivity>>(resultUrl, { method: 'GET' })
            .then((response: IListData<IListActivity>) => { dispatch(new SetActivitiesList(response)); });
    };
}
