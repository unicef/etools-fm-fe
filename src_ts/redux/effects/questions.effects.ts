...
export function loadQuestions(params: IRouteQueryParams): (dispatch: Dispatch) => Promise<void> {
    return (dispatch: Dispatch) => {
        ...
        return request<IListData<IQuestion>>(resultUrl, { method: 'GET' })
            .then((response: IListData<IQuestion>) => { dispatch(new SetQuestionsList(response)); });
    };
}
...
function startRequest(dispatch: Dispatch, url: string, method: RequestMethod, data: IEditedQuestion | {}): Promise<void> {
    ...
    return request(url, options)
        ...
        .then(() => { dispatch(new SetQuestionUpdateState(false)); });
}
