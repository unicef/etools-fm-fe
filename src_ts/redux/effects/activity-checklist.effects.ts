...
export function updateActivityChecklist(id: number, data: Partial<IChecklistItem>[]): (dispatch: Dispatch) => Promise<void> {
    return (dispatch: Dispatch) => {
        const { url }: IResultEndpoint = getEndpoint(ACTIVITY_CHECKLIST, { id });
        return request<IListData<IChecklistItem>>(url, { method: 'PATCH', body: JSON.stringify(data) })
            .then(() => { dispatch<AsyncEffect>(loadActivityChecklist(id)); });
    };
}
